...
?>
<?= Html::beginForm(['/baocao/baocaothongke/showview'], 'post',['id' => 'formShowviewReport','name' => 'formShowviewReport']) ?>                
<!-- Main content -->
<style>
    .col-md-5{
        width: 40%;
...
<section class="content">
    <div class="box box-default">
        <div class="box-body">
            <div class="row">
                <h3>Từ ngày <?=$tuNgay?> Đến ngày <?=$denNgay?>
                    <?php
                    if($daXoa ==1){
                        echo "đã xóa tại";
                    }elseif(($ngayChinhSuaTu!=null && $ngayChinhSuaTu!="")||($ngayChinhSuaDen!=null && $ngayChinhSuaTu!="")){
                        echo "đã chỉnh sửa tại";
                    }
                    ?>: Đã tiếp nhận <?=$countDaTiepNhan?> văn bản - Đang biên tập <?=$countBienTap?> văn bản - Đã ban hành <?=$countBanHanh?> văn bản</h3>
                <div class="col-md-12">
...
<?php
foreach ($selectCoQuan as $v) {
  echo '<input type="hidden" name="selectCoQuan[]" value="' .$v. '" />';
}
foreach ($selectLoaiVanBan as $v) {
  echo '<input type="hidden" name="selectLoaiVanBan[]" value="' .$v. '" />';
}
?>
<?=Html::endForm() ?>
<p id="lasttext"></p>
<script type="text/javascript">
    ...
    function pageClick(page){
        document.formShowviewReport.action = '/baocao/baocaothongke/showview?page='+(page+1);
        document.formShowviewReport.submit();
    }    
    function submitForm(){
        document.formShowviewReport.action = '/baocao/baocaothongke/showview';
        document.formShowviewReport.submit();
    }
</script>