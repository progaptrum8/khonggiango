...
class DanhMucSanPhamController extends BaseController {
    ...
    public function actionIndex() {
        ...
        $query = DanhMucSanPham::find()->where($stringCondition, $arrayCondition);
        ...
        try {
            ...
            if(is_array($idSelected)){
                foreach ($idSelected as $id){
                    $item = DanhMucSanPham::findOne($id);
                    $item->delete();    
                }
            }else{
                $item = DanhMucSanPham::findOne($idSelected);
                $item->delete();
            }
        } catch (\yii\base\Exception $e) {
            Yii::$app->getSession()->setFlash('error', 'Xóa dữ liệu không thành công!');
            return $this->redirect("/danhmuc/danh-muc-san-pham/index");
        }
    ...
    public function actionInput() {
        $item = new DanhMucSanPham();
        $id = (int)trim(Yii::$app->request->get("id"));
        if ((int) $id > 0) {
            $itemFind = DanhMucSanPham::findOne($id);
            if ($itemFind != null) {
                $item = $itemFind;
    ...
    public function actionSave() {
        ...
        $model = new DanhMucSanPham();
        if ((int) $id > 0) {
            $model = DanhMucSanPham::findOne($id);
        }
